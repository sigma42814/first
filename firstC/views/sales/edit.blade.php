    ...
    <div class="card shadow-sm">
        ...
        <div class="card-body p-3">
            ...
            <div class="row invoice-info mb-3">
                ...
                <div class="col-md-1 position-relative">
                    ...
                    <input type="text" id="code" name="code" class="form-control form-control-sm" 
                           ...
                           oninput="debouncedSearchCode(this)" 
                ...
                <div class="col-md-3">
                    ...
                    <input type="text" id="customerName" name="customer_name" 
                           ...
                           value="{{ $sale->customer->name ?? 'No Customer' }}" readonly>
                </div>
                ...
                <div class="col-md-2">
                    ...
                    <input type="date" id="invoiceDate" name="invoice_date" 
                           class="form-control form-control-sm" 
                           value="{{ $sale->invoice_date->format('Y-m-d') }}" readonly>
                </div>
                ...
                <div class="col-md-2">
                    ...
                    <input type="text" id="address" name="address" 
                           class="form-control form-control-sm" 
                           value="{{ $sale->customer->address ?? 'No Address' }}" readonly>
                </div>
            </div>
            ...
            <table id="salesTable" class="table table-bordered table-sm">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="position-relative">
                                <input type="text" class="form-control form-control-sm itemSearch" 
                        ...
                        <td><input type="text" name="batch_number[]" class="batchNumber form-control form-control-sm" 
                                   value="{{ $item->batch_number ?? '' }}" style="border: none;"></td>
            ...
            <div class="btn-group mt-3">
                <button type="button" class="btn btn-primary btn-sm" onclick="updateSale()">
            ...
            <div class="summary mt-3">
                ...
                <p>Total Sales: <span id="totalSales">{{ number_format($sale->total_sales, 2) }}</span> USD</p>
            </div>
            ...
            <div class="balance-section mt-3">
                <div class="row">
                    <div class="col-md-4">
                        ...
                        <input type="number" id="netPayable" name="net_payable" 
                               class="form-control form-control-sm" 
                               value="{{ number_format($sale->net_payable, 2) }}">
                    </div>
                    <div class="col-md-4">
                        <label for="prevBalance" class="small">Previous Balance</label>
                        <input type="number" id="prevBalance" name="prev_balance" 
                               class="form-control form-control-sm" 
                               value="{{ number_format($sale->prev_balance, 2) }}">
                    </div>
                    <div class="col-md-4">
                        <label for="totalBalance" class="small">Total Balance</label>
                        <input type="number" id="totalBalance" name="total_balance" 
                               class="form-control form-control-sm" 
                               value="{{ number_format($sale->total_balance, 2) }}" disabled>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    async function searchCode(input) {
        ...
        if (query.length > 0) {
            try {
                const response = await fetch(`/search-customers?query=${encodeURIComponent(query)}`);
    ...
    async function searchItemCode(input) {
        ...
        if (query.length > 0) {
            try {
                ...
                showAlert('Error fetching items', 'danger');
            }
        } else {
            const dropdown = document.getElementById('itemCodeDropdown');
            if (dropdown) dropdown.style.display = 'none';
    ...
    async function updateSale() {
        ...
        if (salesData.length === 0) {
            showAlert('Please add at least one valid item to the sale.', 'danger');
            return;
        }
