...
<div class="container-fluid px-2">
    <div class="card shadow-sm">
        <div class="card-header bg-primary text-white py-2">
            <h5 class="mb-0">
                <i class="fas fa-undo me-2"></i> Purchase Returns List
            </h5>
        </div>
        <div class="card-body p-3">
            <div class="d-flex justify-content-between mb-4">
                <!-- Add New Purchase Return Button -->
                <a href="{{ route('purchase-returns.create') }}" class="btn btn-success btn-sm">
                    <i class="fas fa-plus me-1"></i> New Return
                </a>
                <!-- Search Bar -->
                <div class="w-50">
                    <input type="text" class="form-control" id="searchReturn" placeholder="Search by return number or company...">
                </div>
            </div>
            <!-- Returns List Table -->
                ...
                <table class="table table-sm table-hover mb-0">
                    ...
                    <tbody id="returnTableBody" class="small">
                        ...
                        <tr>
                            ...
                            <td>{{ $return->return_date ? \Carbon\Carbon::parse($return->return_date)->format('d/m/Y') : 'N/A' }}</td>
                            <td>{{ \Carbon\Carbon::parse($return->updated_at)->format('d/m/Y H:i') }}</td>
                            <td>{{ $return->company->name ?? 'N/A' }}</td>
                            <td class="text-start">{{ number_format($return->total_return, 2) }}</td>
                            <td>
                                <div class="btn-group btn-group-sm">
                                    <a href="{{ route('purchase-returns.edit', $return->id) }}" class="btn btn-primary" title="Edit">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('purchase-returns.destroy', $return->id) }}" method="POST" class="d-inline delete-form">
                                        @csrf
                                        @method('DELETE')
                                    ...
                                    <a href="{{ route('purchase-returns.print', $return->id) }}" class="btn btn-secondary" title="Print">
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title" id="returnDetailsModalLabel">Purchase Return Details</h5>
...
<script>
    ...
    document.getElementById('searchReturn').addEventListener('input', function () {
        ...
        rows.forEach(row => {
            ...
            row.style.display = (returnNum.includes(searchTerm) || companyName.includes(searchTerm)) ? '' : 'none';
        });
    });
    ...
    function showReturnDetails(returnId) {
        fetch(`/purchase-returns/${returnId}/details`)
